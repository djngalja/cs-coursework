...
from domain.service.service_interface import ServiceInterface
...
class ServiceImpl(ServiceInterface):
        ...
        for i in range(ServiceImpl.SZ):
            for j in range(ServiceImpl.SZ):
                ...
                if (prev_state[i][j] == ServiceImpl.EMPTY):
                    if state[i][j] == ServiceImpl.PLAYER1:
                        cnt_changes += 1
    ...
    def game_over_check(self, current_game: CurrentGame) -> bool:
        ...
        if self._evaluate(state) != 0:
            return True
        return not self._moves_left(state)
    ...
    def new_game(self) -> CurrentGame:
        ...
        return CurrentGame(uuid.uuid4(), new_board)
